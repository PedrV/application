...
*/
        ...
        byte[] iv = SymmetricEncryption.createInitializationVector();
        byte[] simetric = SymmetricEncryption.doAESEncryption(str,key,iv);
        ...
        byte[] keyencryption = AsymmetricEncryption.encrypt(key.getEncoded());

        System.out.print("Key Encryption: ");
        for(byte a : keyencryption) System.out.print((char) a);
        System.out.println();

        byte[] msgAssimEncryption = AsymmetricEncryption.encrypt(simetric);

        System.out.print("Encrypted Message Asymmetric Encription: ");
        for(byte a : msgAssimEncryption) System.out.print((char) a);
        System.out.println();

        // Desencrypt process
        byte[] newkey = AsymmetricEncryption.decrypt(keyencryption);
        String newstring = SymmetricEncryption.doAESDecryption(simetric,new SecretKeySpec(newkey, 0, newkey.length, "AES"),iv);
